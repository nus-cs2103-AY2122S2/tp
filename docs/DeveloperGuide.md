...
Cons:
- Increased coupling. For example, originally, `AddXXXCommand`s look for the patient in the latest filtered list of patients to add new entities to. Since at the summary screen the list is in filtered state, we cannot add to other patients apart from one being viewed. `AddXXXCommand`s had to be modified as a result.
