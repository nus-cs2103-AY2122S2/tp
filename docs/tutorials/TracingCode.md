...
According to the sequence diagram you saw earlier (and repeated above for reference), the `UI` component yields control to the `Logic` component through a method named `execute`. Searching through the code base for an `execute()` method that belongs to the `Logic` component yields a promising candidate in `seedu.tinner.logic.Logic`.
...
A quick look at the `seedu.tinner.logic.Logic` (an extract given below) confirms that this indeed might be what we’re looking for.
...
1. It appears that a `JsonAdaptedCompany` is created for each `Company` and then added to the `JsonSerializableAddressBook`.
