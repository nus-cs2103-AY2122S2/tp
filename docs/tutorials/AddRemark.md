...
This command allows users of the AddressBook application to add optional remarks to people in WoofAreYou and edit it if required. The command should have the following format:
...
/**
 * Changes the remark of an existing pet in WoofAreYou.
 */
...
``` java
    public static final String MESSAGE_USAGE = COMMAND_WORD
            + ": Edits the remark of the pet identified "
...
public class RemarkCommand extends Command {
    ...
    /**
     * @param index of the pet in the filtered pet list to edit the remark
     * @param remark of the pet to be updated to
     */
...
Create a `RemarkCommandParser` class in the `woofareyou.logic.parser` package. The class must extend the `Parser` interface.
...
Now that we have all the information that we need, let’s lay the groundwork for propagating the remarks added into the in-memory storage of pet data. We achieve that by working with the `Person` model. Each field in a Person is implemented as a separate class (e.g. a `Name` object represents the pet’s name). That means we should add a `Remark` class so that we can use a `Remark` object to represent a remark given to a pet.

### Add a new `Remark` class

Create a new `Remark` in `woofareyou.model.pet`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
...
Without getting too deep into `fxml`, let’s go on a 5 minute adventure to get some placeholder text to show up for each pet.

Simply add the following to [`woofareyou.ui.PetCard`](https://github.com/se-edu/addressbook-level3/commit/850b78879582f38accb05dd20c245963c65ea599#diff-639834f1e05afe2276a86372adf0fe5f69314642c2d93cfa543d614ce5a76688).
...
:exclamation: You must delete AddressBook’s storage file located at `/data/addressbook.json` before running it! Not doing so will cause AddressBook to default to an empty WoofAreYou!
...
``` java
public PersonCard(Person pet, int displayedIndex) {
    //...
...
//...
    ...
    public CommandResult execute(Model model) throws CommandException {
        ...
        Person petToEdit = lastShownList.get(index.getZeroBased());
        Person editedPet = new Person(
                petToEdit.getName(), petToEdit.getPhone(), petToEdit.getEmail(),
                petToEdit.getAddress(), remark, petToEdit.getTags());

        model.setPerson(petToEdit, editedPet);
     ...
     * {@code petToEdit}.
     */
    private String generateSuccessMessage(Person petToEdit) {
