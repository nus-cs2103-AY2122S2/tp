...
``` java
    public static final String MESSAGE_USAGE = COMMAND_WORD
            + ": Edits the remark of the candidate identified "
...
Now that we have all the information that we need, let’s lay the groundwork for propagating the remarks added into the in-memory storage of candidate data. We achieve that by working with the `Candidate` model. Each field in a Candidate is implemented as a separate class (e.g. a `Name` object represents the candidate’s name). That means we should add a `Remark` class so that we can use a `Remark` object to represent a remark given to a candidate.

### Add a new `Remark` class

Create a new `Remark` in `seedu.address.model.candidate`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
...
Without getting too deep into `fxml`, let’s go on a 5 minute adventure to get some placeholder text to show up for each candidate.

Simply add the following to [`seedu.address.ui.CandidateCard`](https://github.com/se-edu/addressbook-level3/commit/850b78879582f38accb05dd20c245963c65ea599#diff-639834f1e05afe2276a86372adf0fe5f69314642c2d93cfa543d614ce5a76688).
...
Then insert the following into [`main/resources/view/CandidateListCard.fxml`](https://github.com/se-edu/addressbook-level3/commit/850b78879582f38accb05dd20c245963c65ea599#diff-d44c4f51c24f6253c277a2bb9bc440b8064d9c15ad7cb7ceda280bca032efce9).

**`CandidateListCard.fxml`:**
...
## Modify `Candidate` to support a `Remark` field

Since `CandidateCard` displays data from a `Candidate`, we need to update `Candidate` to get our `Remark` displayed!

### Modify `Candidate`

We change the constructor of `Candidate` to take a `Remark`. We will also need to define new fields and accessors accordingly to store our new addition.

### Update other usages of `Candidate`

Unfortunately, a change to `Candidate` will cause other commands to break, you will have to modify these commands to use the updated `Candidate`!

<div markdown="span" class="alert alert-primary">

:bulb: Use the `Find Usages` feature in IntelliJ IDEA on the `Candidate` class to find these commands.
...
AddressBook stores data by serializing `JsonAdaptedCandidate` into `json` with the help of an external library — Jackson. Let’s update `JsonAdaptedCandidate` to work with our new `Candidate`!
...
Now that we have finalized the `Candidate` class and its dependencies, we can now bind the `Remark` field to the UI.
...
In this last step, we modify `RemarkCommand#execute()` to change the `Remark` of a `Candidate`. Since all fields in a `Candidate` are immutable, we create a new instance of a `Candidate` with the values that we want and
save it with `Model#setCandidate()`.
...
//...
    ...
    public CommandResult execute(Model model) throws CommandException {
        ...
        Candidate editedCandidate = new Candidate(
                candidateToEdit.getName(), candidateToEdit.getPhone(), candidateToEdit.getEmail(),
                candidateToEdit.getAddress(), remark, candidateToEdit.getTags());
