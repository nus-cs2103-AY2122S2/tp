...
Upon click, the default OS file picker should show up, with filters for only .csv and .json files to be accepted. Accessing import via the top menu will allow users to import their task and appointments list as JSON/CSV files that were generated by another instance of ContaX.
...
**List** | `list`
**Help** | `help`
**ExportCSV** | `exportcsv [EXPORTTYPE]`<br> e.g., `exportcsv 2`
**ImportCSV** | `importcsv [n/COLUMNNUM] [p/COLUMN_PERSON] [e/COLUMN_EMAIL] [a/COLUMN_ADDRESS] [t/COLUMN_TAGS]` <br> e.g., `importCSV n/2 p/3 e/5 a/6 t/4`
