...
public class AddCommandParser implements Parser<AddCommand> {
    ...
    public AddCommand parse(String args) throws ParseException {
        ...
        if (!arePrefixesPresent(argMultimap, PREFIX_NAME, PREFIX_EXPIRY_DATE, PREFIX_DESCRIPTION, PREFIX_PRICE)
        ...
        ExpiryDate email = ParserUtil.parseExpiryDate(argMultimap.getValue(PREFIX_EXPIRY_DATE).get());
        Description address = ParserUtil.parseDescription(argMultimap.getValue(PREFIX_DESCRIPTION).get());
        Price price = ParserUtil.parsePrice(argMultimap.getValue(PREFIX_PRICE).get());

        Product product = new Product(name, category, email, address, price);
