...
public class EditCommandParser implements Parser<EditCommand> {
    ...
    /**
     * Parses {@code Collection<String> activities} into a {@code Set<Activity>} if {@code activities} is non-empty.
     * If {@code activities} contain only one element which is an empty string, it will be parsed into a
     * {@code Set<Activity>} containing zero activities.
     */
    private Optional<Set<Activity>> parseActivitiesForEdit(Collection<String> activities) throws ParseException {
        assert activities != null;

        if (activities.isEmpty()) {
            return Optional.empty();
        }
        Collection<String> activitySet = activities.size() == 1
                && activities.contains("")
