...
public class AddCommandParserTest {
    ...
    public void parse_allFieldsPresent_success() {
        ...
        assertParseSuccess(parser, PREAMBLE_WHITESPACE + NAME_DESC_ALICE_IN_WONDERLAND + STATUS_DESC_COMPLETED + TAG_DESC_MOVIE, new AddCommand(expectedShow));

        // multiple names - last name accepted
        assertParseSuccess(parser, NAME_DESC_GONE + NAME_DESC_ALICE_IN_WONDERLAND +  STATUS_DESC_COMPLETED + TAG_DESC_MOVIE, new AddCommand(expectedShow));

        // multiple status - last status accepted
        assertParseSuccess(parser, NAME_DESC_ALICE_IN_WONDERLAND + STATUS_DESC_WATCHING + STATUS_DESC_COMPLETED + TAG_DESC_MOVIE, new AddCommand(expectedShow));

        // multiple tags - all accepted
        Show expectedShowMultipleTags = new ShowBuilder(GONE).withTags(VALID_TAG_OWESMONEY, VALID_TAG_FRIENDS)
                .build();
        assertParseSuccess(parser, NAME_DESC_GONE + STATUS_DESC_WATCHING + TAG_DESC_OWESMONEY + TAG_DESC_FRIENDS, new AddCommand(expectedShowMultipleTags));
    }
    ...
    public void parse_invalidValue_failure() {
        // invalid name
        assertParseFailure(parser, INVALID_NAME_DESC + STATUS_DESC_COMPLETED + TAG_DESC_MOVIE, Name.MESSAGE_CONSTRAINTS);

        // invalid status
        assertParseFailure(parser, NAME_DESC_ALICE_IN_WONDERLAND + INVALID_STATUS_DESC + TAG_DESC_MOVIE, Status.MESSAGE_CONSTRAINTS);

        // invalid tag
        assertParseFailure(parser, NAME_DESC_ALICE_IN_WONDERLAND + STATUS_DESC_COMPLETED + INVALID_TAG_DESC, Tag.MESSAGE_CONSTRAINTS);
        ...
        // non-empty preamble
        assertParseFailure(parser, PREAMBLE_NON_EMPTY + NAME_DESC_ALICE_IN_WONDERLAND + STATUS_DESC_COMPLETED + TAG_DESC_MOVIE,
