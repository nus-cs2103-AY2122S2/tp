...
public class EditAppointmentCommandParserTest {
    ...
    public void parse_missingParts_failure() {
        // no index specified
        assertParseFailure(parser, APPOINTMENT_NAME_ALONE, MESSAGE_INVALID_FORMAT);

        // no field specified
        assertParseFailure(parser, VALID_INDEX, EditAppointmentCommand.MESSAGE_NOT_EDITED);
    ...
    public void parse_invalidPreamble_failure() {
        // negative index
        assertParseFailure(parser, NEGATIVE_INDEX + APPOINTMENT_NAME_ALONE, MESSAGE_INVALID_FORMAT);

        // zero index
        assertParseFailure(parser, ZERO_INDEX + APPOINTMENT_NAME_ALONE, MESSAGE_INVALID_FORMAT);

        // invalid arguments being parsed as preamble
        assertParseFailure(parser, VALID_INDEX + " some random string", MESSAGE_INVALID_FORMAT);

        // invalid prefix being parsed as preamble
        assertParseFailure(parser, VALID_INDEX + " i/ string", MESSAGE_INVALID_FORMAT);
    }

    @Test
    public void parse_invalidValue_failure() {
        // Various invalid fields
        assertParseFailure(parser, VALID_INDEX + INVALID_APPOINTMENT_NAME_DESC, Name.MESSAGE_CONSTRAINTS);
        assertParseFailure(parser, VALID_INDEX + INVALID_DATE_DESC, Messages.MESSAGE_INVALID_DATE);
        assertParseFailure(parser, VALID_INDEX + INVALID_TIME_DESC, Messages.MESSAGE_INVALID_TIME);
        assertParseFailure(parser, VALID_INDEX + INVALID_APPOINTMENT_DURATION_DESC, Duration.MESSAGE_CONSTRAINTS);
        assertParseFailure(parser, VALID_INDEX + INVALID_APPOINTMENT_PERSON, ParserUtil.MESSAGE_INVALID_INDEX);

        // Parsing order left to right test, stop upon first failure
        assertParseFailure(parser,
                VALID_INDEX + INVALID_APPOINTMENT_NAME_DESC + APPOINTMENT_TIME, Name.MESSAGE_CONSTRAINTS);
        assertParseFailure(parser,
                VALID_INDEX + APPOINTMENT_NAME_ALONE + INVALID_DATE_DESC, Messages.MESSAGE_INVALID_DATE);

        // multiple invalid values, but parser stops at first invalid
        assertParseFailure(parser, VALID_INDEX + INVALID_APPOINTMENT_NAME_DESC + INVALID_DATE_DESC,
    ...
    public void parse_oneFieldSpecified_success() {
        ...
        // name
        String userInput = VALID_INDEX + APPOINTMENT_NAME_ALONE;
        EditAppointmentDescriptor descriptor = new EditAppointmentDescriptorBuilder()
        ...
        // start date
        userInput = VALID_INDEX + APPOINTMENT_DATE;
        ...
        // start time
        userInput = VALID_INDEX + APPOINTMENT_TIME;
        descriptor = new EditAppointmentDescriptorBuilder().withStartTime(VALID_TIME).build();
        expectedCommand = new EditAppointmentCommand(targetIndex, descriptor);
        assertParseSuccess(parser, userInput, expectedCommand);

        // duration
        userInput = VALID_INDEX + APPOINTMENT_DURATION;
        ...
        // person
        userInput = VALID_INDEX + APPOINTMENT_FIRST_PERSON;
        descriptor = new EditAppointmentDescriptorBuilder()
                .withPerson(INDEX_FIRST_PERSON).build();
        expectedCommand = new EditAppointmentCommand(targetIndex, descriptor);
        assertParseSuccess(parser, userInput, expectedCommand);
        ...
        // remove person
        userInput = VALID_INDEX + APPOINTMENT_REMOVE_PERSON;
        descriptor = new EditAppointmentDescriptorBuilder()
    ...
    @Test
    public void parse_duplicateFieldsWithValidInputFirst_failure() {
        assertParseFailure(parser, VALID_INDEX + APPOINTMENT_NAME_ALONE + INVALID_APPOINTMENT_NAME_DESC,
                Name.MESSAGE_CONSTRAINTS);
        assertParseFailure(parser, VALID_INDEX + APPOINTMENT_DATE + INVALID_DATE_DESC,
                Messages.MESSAGE_INVALID_DATE);
        assertParseFailure(parser, VALID_INDEX + APPOINTMENT_TIME + INVALID_TIME_DESC,
                Messages.MESSAGE_INVALID_TIME);
        assertParseFailure(parser, VALID_INDEX + APPOINTMENT_DURATION + INVALID_APPOINTMENT_DURATION_DESC,
                Duration.MESSAGE_CONSTRAINTS);
        assertParseFailure(parser, VALID_INDEX + APPOINTMENT_FIRST_PERSON + INVALID_APPOINTMENT_PERSON,
