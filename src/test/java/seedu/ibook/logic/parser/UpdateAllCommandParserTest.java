...
public class UpdateAllCommandParserTest {

    private static final String MESSAGE_INVALID_FORMAT =
            String.format(MESSAGE_INVALID_COMMAND_FORMAT, UpdateAllCommand.MESSAGE_USAGE);

    private UpdateAllCommandParser parser = new UpdateAllCommandParser();

    @Test
    public void parse_missingParts_failure() {
        // no index and no field specified
        assertParseFailure(parser, "", UpdateAllCommand.MESSAGE_NOT_UPDATED);
    }
    ...
    public void parse_invalidValue_failure() {
        ...
        // invalid price
        assertParseFailure(parser, INVALID_PRICE_DESC, Price.MESSAGE_CONSTRAINTS);

        // invalid name followed by valid category
        assertParseFailure(parser, INVALID_NAME_DESC + CATEGORY_FULL_A, Name.MESSAGE_CONSTRAINTS);

        // valid price followed by invalid price. The test case for invalid price followed by valid price
        // is tested at {@code parse_invalidValueFollowedByValidValue_success()}
        assertParseFailure(parser, PRICE_FULL_A + INVALID_PRICE_DESC, Price.MESSAGE_CONSTRAINTS);

        // multiple invalid values, but only the first invalid value is captured
        assertParseFailure(parser, INVALID_NAME_DESC + INVALID_EXPIRY_DATE_DESC + VALID_PRICE_A,
                Name.MESSAGE_CONSTRAINTS);
    }

    @Test
    public void parse_allFieldsSpecified_success() {
        String userInput = NAME_FULL_A + CATEGORY_FULL_A
                + DESCRIPTION_FULL_A + PRICE_FULL_A;

        UpdateProductDescriptor descriptor = new UpdateProductDescriptorBuilder()
                .withName(VALID_NAME_A)
                .withCategory(VALID_CATEGORY_A)
                .withDescription(VALID_DESCRIPTION_A)
                .withPrice(VALID_PRICE_A).build();
        UpdateAllCommand expectedCommand = new UpdateAllCommand(descriptor);
    ...
    public void parse_someFieldsSpecified_success() {
        String userInput = NAME_FULL_A + DESCRIPTION_FULL_A;
    ...
    public void parse_invalidValueFollowedByValidValue_success() {
        ...
        String userInput = INVALID_PRICE_DESC + PRICE_FULL_B;
        UpdateProductDescriptor descriptor = new UpdateProductDescriptorBuilder().withPrice(VALID_PRICE_B).build();
        UpdateAllCommand expectedCommand = new UpdateAllCommand(descriptor);
        assertParseSuccess(parser, userInput, expectedCommand);

        // other valid values specified
        userInput = NAME_FULL_B + INVALID_PRICE_DESC + DESCRIPTION_FULL_B + PRICE_FULL_B;
